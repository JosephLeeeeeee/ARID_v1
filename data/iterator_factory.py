...
def get_arid_flow(data_root='./dataset/ARID',
    ...
    val_sampler = sampler.SequentialSampling(num=clip_length, interval=val_interval, fix_cursor=True, shuffle=True)
    # val_sampler = sampler.SegmentalSampling(num_per_seg=clip_length, segments=segments, interval=val_interval,
    # fix_cursor=True, shuffle=True)
...
def creat(name, batch_size, use_flow, num_workers=8, **kwargs):  # Change num_worker here to fit your PC
